...
import { Mechanic, Service, Car } from "@/app/api/types";
import { httpClient } from "@/app/api/httpClient";
import { API_CONFIG } from "@/app/api/config";
import { DateRangePicker } from "@/components/DateRangePicker";
import { useAuth } from "@/app/api";

type ClientReservationFilter = {
  dateFrom?: string;
  dateTo?: string;
  carId?: number;
  serviceId?: number;
...
type ClientReservationFilterModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filter: ClientReservationFilter) => void;
  currentFilter?: ClientReservationFilter;
};

export function ClientReservationFilterModal({
  isOpen,
  onClose,
  onApply,
  currentFilter,
}: ClientReservationFilterModalProps) {
  const { user } = useAuth();
  ...
  const [filter, setFilter] = useState<ClientReservationFilter>({
  ...
  const handleApply = () => {
    const filterToSend: ClientReservationFilter = {
