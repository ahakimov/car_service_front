...
type MechanicRepairJobFilterModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filter: MechanicRepairJobFilter) => void;
  currentFilter?: MechanicRepairJobFilter;
};

export function MechanicRepairJobFilterModal({
  isOpen,
  onClose,
  onApply,
  currentFilter,
}: MechanicRepairJobFilterModalProps) {
  const { user } = useAuth();
  ...
  const [filter, setFilter] = useState<MechanicRepairJobFilter>({
  ...
  const handleApply = () => {
    const filterToSend: MechanicRepairJobFilter = {
