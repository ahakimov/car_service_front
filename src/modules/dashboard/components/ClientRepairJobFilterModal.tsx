...
type ClientRepairJobFilterModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filter: ClientRepairJobFilter) => void;
  currentFilter?: ClientRepairJobFilter;
};

export function ClientRepairJobFilterModal({
  isOpen,
  onClose,
  onApply,
  currentFilter,
}: ClientRepairJobFilterModalProps) {
  const { user } = useAuth();
  ...
  const [filter, setFilter] = useState<ClientRepairJobFilter>({
  ...
  const handleApply = () => {
    const filterToSend: ClientRepairJobFilter = {
